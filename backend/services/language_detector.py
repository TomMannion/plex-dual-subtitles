...
class SimpleLanguageDetector:
    ...
    def detect_from_file(self, file_path: Path, declared_lang: Optional[str] = None) -> LanguageDetectionResult:
        ...
        if not settings.app.enable_language_detection:
            ...
            logger.debug(f"Language detection disabled, using: {lang.value}")
        ...
        try:
            ...
            logger.debug(
                f"Language detected: {result.detected_language.value} "
                f"(confidence={result.confidence:.2f}, method={result.method_used})"
            )
            return result
            
        except Exception as e:
            logger.warning(f"Language detection error for {file_path}: {e}")
            if declared_lang:
