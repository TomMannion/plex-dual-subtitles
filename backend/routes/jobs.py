...
from services.bulk_processor import process_bulk_dual_subtitles
...
async def get_job(job_id: str):
    ...
    try:
        ...
        logger.error(f"Error serializing job {job_id}: {e}")
...
async def create_bulk_dual_subtitle_job(request: CreateBulkJobRequest, background_tasks: BackgroundTasks, http_request: Request):
    ...
    logger.debug(f"Received Plex token: {plex_token[:20] if plex_token else 'None'}...")
    if not plex_token:
    ...
    def bulk_subtitle_job(job_id: str, progress_callback, **params):
        ...
        return process_bulk_dual_subtitles(
            job_id=job_id,
